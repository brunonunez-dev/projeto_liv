...
SEARCH_CACHE_FILE = "search_cache.json" # Novo arquivo de cache para buscas
...
# --- GERENCIAMENTO DE CACHE (DETALHES E TRADUÇÕES) ---
...
# --- GERENCIAMENTO DE CACHE (RESULTADOS DE BUSCA) ---

def get_search_cache():
    """Carrega o cache de buscas do arquivo JSON."""
...
def update_search_cache(query, results):
    """Salva o resultado de uma busca no arquivo JSON."""
...
def fetch_books_smart(query_en):
    """Busca livros primeiro no JSON, depois na API."""
    query_key = unidecode(query_en.lower().strip())
    search_cache = get_search_cache()
    
    # 1. Tenta buscar no cache JSON
    if query_key in search_cache:
        return search_cache[query_key]["results"]
    
    # 2. Se não estiver no cache, vai para a API
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Salva no cache JSON para a próxima vez
            if docs:
...
if (submit_button or 'current_query' in st.session_state) and query_pt:
    ...
    with status_placeholder.status("Buscando no acervo...", expanded=False) as s:
        # Agora usamos a função que verifica o cache JSON de buscas
    ...
    if results:
        valid_results = [d for d in results if d.get("cover_i")]
