...
def fetch_books_smart(query_raw):
    query_key = unidecode(query_raw.lower().strip())
...
if (submit_button or 'current_query' in st.session_state) and query_pt:
    ...
    # AQUI ESTAVA A TRADUÇÃO. AGORA A QUERY VAI DIRETO:
    query_para_busca = query_pt
    
    status_placeholder = st.empty()
    
    with status_placeholder.status("Buscando no acervo...", expanded=False) as s:
        results = fetch_books_smart(query_para_busca)
    ...
    if results:
        ...
        for i, doc in enumerate(results[:st.session_state.display_limit]):
